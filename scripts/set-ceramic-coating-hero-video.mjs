/**
 * Point the 1-year ceramic coating service's heroVideo at the studio's own saved reel.
 *
 * `1-year-ceramic-coating` ("1 Year Ceramic Coating") — matched on id, slug AND title
 * before anything is written, the same discipline scripts/set-service-card-images.mjs
 * uses, because the database holds inactive rows that reuse active titles. The video is
 * the studio's own Instagram reel (instagram.com/reel/DcBSRiSCja4/), downloaded (the
 * signed CDN URL Instagram serves expires) and saved under the git-tracked
 * attached_assets/reels/ tree — the same "download once, serve from the repo forever"
 * approach set-service-card-images.mjs documents for photos. Trimmed to the last real
 * footage frame (18.9s of the original ~22.8s) so playback ends before the reel's own
 * black "Premium" title card and closing phone-number card — neither belongs on a
 ...
 *
 *   CARCARE_TARGET_DATABASE_URL="$CARCARE_DATABASE_URL" node scripts/set-ceramic-coating-hero-video.mjs
 *   CARCARE_TARGET_DATABASE_URL="$CARCARE_DATABASE_URL" node scripts/set-ceramic-coating-hero-video.mjs --apply
 *
...
const TARGET = {
  ...
  file: '1-year-ceramic-coating-hero.mp4',
};
...
try {
  ...
  if (row.hero_video === url) {
    ...
    if (APPLY) {
      ...
      const backupPath = path.join(repoRoot, '..', '..', 'ceramic-coating-hero-video-backup.json');
      fs.writeFileSync(
        backupPath,
